...
def main():
    ...
    val_set = torchvision.datasets.CIFAR10(root='./dataset', train=False, download=False)
    ...
    if config.vis_only:
        model = torch.load(os.path.join(ckpt_path, "ckpt250.pth"),weights_only=False)
        model.eval()
        ...
        wandb.log({"debug_recons": [wandb.Image(img) for img in merged]})
    ...
    for i in range(config.epochs):
        ...
        wandb.log({"train loss":sum(train_loss)/len(train_loss), "val loss":val_loss, "lr":optimizer.param_groups[0]['lr'],"reconstructions": [wandb.Image(img) for img in merged]})
