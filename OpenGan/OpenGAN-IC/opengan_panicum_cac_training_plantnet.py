...
def val(netD,dataloader_val_closedset,dataloader_uuc_val):
    netD.eval()
    ...
    i = 0
    count = 0
    for X,y in dataloader_val_closedset:    
...
project_name = 'OpenGan_Panicum_plantnet_cac' + modelFlag   # we save all the checkpoints in this directory
...
path_to_feats = os.path.join(NOMES.FEATS_DIR.value,"Panicum_plantnet_cac",modelFlag) # the path to cached off-the-shelf features
...
exp_dir = os.path.join("/home/alexandreselani/Desktop/OpenGan/OpenGAN-IC/Experimentos/Panicum_plantnet_cac/",modelFlag)

print(path_to_feats)

transform_train = transforms.Compose([
    transforms.Resize((320, 320)), # Mantém proporção 2:1 (H, W)
    #transforms.RandomHorizontalFlip(),
    #transforms.RandomVerticalFlip(),
    #transforms.RandomRotation(90), # Essencial para drone
    #transforms.ColorJitter(brightness=0.1, contrast=0.1), # Lida com luz do sol
    transforms.ToTensor(),
...
for iteration in range(5):
    ...
    data_manager = Panicum_halfsize_loader(bs=batch_size)
    dataloader = data_manager.load_train(iteration,transform_train)
    

    feat_extractor = ResNet18_cac_feature_extraction(2)
    feat_extractor.load_model(torch.load(os.path.join("/home/alexandreselani/Desktop/Experimento_panicum_cac/ResNet18",f"Fold_{iteration}",f"ResNet18_Panicum_cac_fold_{iteration}_plantnet.pt")))
    feat_extractor.model.eval()
    

    uuc_val  = torch.load(os.path.join(path_to_feats,f'Fold_{iteration}',"uuc_val_features.pt"))
    uuc_val_dataset = FeatDataset(uuc_val)
    dataloader_uuc_val = DataLoader(uuc_val_dataset, batch_size=batch_size, shuffle=True, num_workers=4)

    kkc_val  = torch.load(os.path.join(path_to_feats,f'Fold_{iteration}',"kkc_val_features.pt"))
    kkc_val_dataset = FeatDataset(kkc_val)
    dataloader_val_closedset = DataLoader(kkc_val_dataset, batch_size=batch_size, shuffle=True, num_workers=4)
    ...
    for epoch in range(num_epochs):
        ...
        for i, (data,y) in enumerate(dataloader, 0):
            ...
            # Format batch
            data = data.to(device)
            extracted_features = feat_extractor.extract_features(data)
            # Adiciona um ruído suave (Instance Noise) que decai com o tempo
            # Isso torna a tarefa do Discriminador menos trivial
            real_cpu = extracted_features 
        ...
        ###test discriminator
        epoch_roc = val(netD,dataloader_val_closedset=dataloader_val_closedset,dataloader_uuc_val=dataloader_uuc_val)
