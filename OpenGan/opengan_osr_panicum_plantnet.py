...
feats_dir = os.path.join(NOMES.FEATS_DIR.value, "Panicum_plantnet", model_name)

result_dir = os.path.join("/home/alexandreselani/Desktop/OpenGan/Resultados/Panicum_plantnet/", model_name)
...
def create_instances(epsilon, num_classes, fold, best_epoch=None):
    ...
    path_to_D = os.path.join("/home/alexandreselani/Desktop/OpenGan/OpenGAN-IC/Experimentos/Panicum_plantnet", model_name, f"Fold+{fold}", 'best_epoch.DNet')
    ...
    classifier.load_model(torch.load(os.path.join(NOMES.PANICUM_RESNET.value, f"Fold_{fold}", f"ResNet18_Panicum_fold_{fold}_plantnet.pt")))
