...
pca_cac = TSNE(n_components=2, random_state=42)

test_logits_projected = pca_cac.fit_transform(scaled_test_logits)
...
for klass in range(-1, 10, 1):
    ...
    if class_logits.shape[0] > 0:
        plt.scatter(
            ...
            color=target_2_color[klass], label=target_2_name[klass], alpha=0.3
        )

# Fixed Title: Includes individual and total explained variance
plt.title(f"Logits Space - {name} (CAC) - TSNE")
plt.xlabel("PC 1")
plt.ylabel("PC 2")
plt.legend(bbox_to_anchor=(1.05, 1), loc='upper left')
plt.tight_layout()
plt.savefig(os.path.join(save_dir, f"{name}_cac_TSNE.png"))
plt.close()
...
pca_ce = TSNE(n_components=2, random_state=42)

test_logits_projected = pca_ce.fit_transform(scaled_test_logits)
...
# Fixed Title: Includes individual and total explained variance
plt.title(f"Logits Space - {name} (Cross Entropy) - TSNE")
plt.xlabel("PC 1")
plt.ylabel("PC 2")
plt.legend(bbox_to_anchor=(1.05, 1), loc='upper left')
plt.tight_layout()
