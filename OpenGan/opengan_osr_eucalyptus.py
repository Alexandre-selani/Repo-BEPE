...
model_name = "AlexNet"
dataset= "dataset-1"
feats_dir = os.path.join(NOMES.FEATS_DIR.value, "Eucalyptus", dataset, model_name)

result_dir = os.path.join("/home/alexandreselani/Desktop/OpenGan/Resultados/Eucalyptus/",dataset, model_name)
...
def create_instances(epsilon, num_classes, fold, best_epoch=None):
    ...
    path_to_D = os.path.join("/home/alexandreselani/Desktop/OpenGan/OpenGAN-IC/Experimentos/Eucalyptus",dataset, model_name, f"Fold+{fold}", 'best_epoch.DNet')

    discriminator.load_state_dict(torch.load(path_to_D))
    discriminator.eval()

    #---------------------Carregando o classificador
    classifier = AlexNet_feature_extraction(num_classes)
    classifier.load_model(torch.load(os.path.join("/home/alexandreselani/Desktop/Eucalyptus/OpenSet/Models/",dataset, f"AlexNet_fold_{fold}.pt")))
