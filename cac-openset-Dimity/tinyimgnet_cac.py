...
from Utils import *
from Modelos import ResNet18_cac,AlexNet_cac
...
model_name = "AlexNet"
result_dir = os.path.join(f"/home/alexandreselani/Desktop/cac-openset-Dimity/Results/Eucalyptus_cac/",dataset, model_name)
...
def val(epsilons):
    ...
    data_manager = Eucalyptus_openset_loader(bs=32,dataset=dataset)

    for fold in range(5):
        fold_results = []

        trainloader = data_manager.load_train(fold,weights.transforms())
        valloader = data_manager.load_val(fold,weights.transforms())
        
        model = AlexNet_cac(num_classes).to(device)
        model.load_state_dict(torch.load(f"/home/alexandreselani/Desktop/Eucalyptus/OpenSet/Models/CAC/{dataset}/AlexNet_fold_{fold}.pt"))
...
def test(epsilons):
    ...
    for fold in range(5):
        ...
        trainloader = data_manager.load_train(fold,weights.transforms())
        testloader = data_manager.load_test(fold,weights.transforms())
...
if __name__ == "__main__":
    epsilons_val = np.arange(0.0, 0.2, 0.001).tolist()
    epsilons_test = np.arange(0.0, 0.01, 0.001).tolist()
