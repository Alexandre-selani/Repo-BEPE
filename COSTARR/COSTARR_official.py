...
class COSTARR(Postprocessor_Base):
    ...
    def norm(self, logits, FVs):
        ...
        for c in tqdm(self.setup_dict.keys()):
            ...
            scaled_class_logits = torch.minimum(torch.ones(logits[class_mask, c].shape),torch.maximum(torch.zeros(logits[class_mask, c].shape), (logits[class_mask, c]-self.logit_min)/(self.logit_max-self.logit_min)))
    ...
    def setup(self, logits, FV): #ASSUME DATA IS CORRECTLY PREDICTED
        ...
        for c in tqdm(classes):
            ...
            class_models[c] = std, mean, hmean, hstd
