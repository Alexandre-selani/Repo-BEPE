...
project_name = f'Tinyimgnet' + modelFlag   # we save all the checkpoints in this directory
...
path_to_feats = os.path.join(NOMES.FEATS_DIR.value,"Tinyimgnet",modelFlag) # the path to cached off-the-shelf features
...
exp_dir = os.path.join("/home/alexandreselani/Desktop/OpenGan/OpenGAN-IC/Experimentos/Tinyimgnet/",modelFlag)
...
for iteration in range(5):
    ...
    train = torch.load(os.path.join(path_to_feats,f'Split_{iteration}',"train_features.pt"))
    train_dataset = FeatDataset(train)
    dataloader = DataLoader(train_dataset, batch_size=batch_size, shuffle=True, num_workers=4)

    uuc_val  = torch.load(os.path.join(path_to_feats,f'Split_{iteration}',"uuc_val_features.pt"))
    ...
    kkc_val  = torch.load(os.path.join(path_to_feats,f'Split_{iteration}',"kkc_val_features.pt"))
