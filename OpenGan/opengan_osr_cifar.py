...
num_classes = 10
best_epoch = 74 #constatado previamente por meio de uma selecao de modelo do discriminador
bs=256


model_name = NOMES.RESNET18.value
feats_dir = os.path.join(NOMES.FEATS_DIR.value,"CIFAR",model_name)

result_dir = os.path.join("/home/alexandreselani/Desktop/OpenGan/Resultados/Cifar10/",model_name)
...
def create_instances(epsilon,num_classes,best_epoch):
    ...
    path_to_D = os.path.join("/home/alexandreselani/Desktop/OpenGan/OpenGAN-IC/Experimentos/Cifar10/OpenGan_cifar10ResNet18",'epoch-{}.DNet'.format(best_epoch))

    discriminator.load_state_dict(torch.load(path_to_D))

    #---------------------Carregando o classificador
    classifier = ResNet18_32x32_feature_extraction(num_classes)
    classifier.load_model(torch.load(NOMES.RESNET18_CIFAR10.value))
...
def model_selection(epsilons):
    ...
    mnist_val_data = torch.load(os.path.join(feats_dir, "CIFAR_val_features.pt"))
    mnist_val_dataset = FeatDataset(mnist_val_data)

    omni_val_data = torch.load("/home/alexandreselani/Desktop/Features_extraidas/SVHN/ResNet18/SVHN_val_features.pt")
...
def main():
    ...
    mnist_test_data = torch.load(os.path.join(feats_dir, "CIFAR_test_features.pt"))
    mnist_test_dataset = FeatDataset(mnist_test_data)

    omni_test_data = torch.load("/home/alexandreselani/Desktop/Features_extraidas/SVHN/ResNet18/SVHN_test_features.pt")
    ...
    for n in range(10):
        col_labels.append(str(n))
    mc = Matriz_confusao_osr_dataset_outlier(predicts,labels,labels,[],col_labels=col_labels)
    mc.computa_matriz()
    mc.exibe_matriz(dir=os.path.join("/home/alexandreselani/Desktop/OpenGan/Resultados/Cifar10",model_name,f"Cifar_eps_{epsilon}"))
