...
num_classes = 200
best_epoch = 46 #constatado previamente por meio de uma selecao de modelo do discriminador
bs=256
...
def create_instances(epsilon,num_classes,best_epoch):
    ...
    path_to_D = os.path.join("/home/alexandreselani/Desktop/OpenGan/OpenGAN-IC/Experimentos/Tinyimgnet/OpenGan_tinyimgnetResNet18",'epoch-{}.DNet'.format(best_epoch))
    ...
    classifier.load_model(torch.load(NOMES.RESNET18_TINY_IMAGE_NET.value))
...
def model_selection(epsilons):
    ...
    mnist_val_data = torch.load(os.path.join(feats_dir, "val_features.pt"))
...
def main():
    ...
    mnist_test_data = torch.load(os.path.join(feats_dir, "val_features.pt"))
    ...
    #mc.computa_matriz()
    #mc.exibe_matriz(dir=os.path.join("/home/alexandreselani/Desktop/OpenGan/Resultados/Tinyimgnet",model_name,f"Tinyimgnet_eps_{epsilon}"))
