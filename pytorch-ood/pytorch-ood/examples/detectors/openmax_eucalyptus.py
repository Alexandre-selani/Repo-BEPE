...
def grid_search():
    ...
    output_dir = f"/home/alexandreselani/Desktop/pytorch-ood/pytorch-ood/experimento_eucalyptus/{dataset}/Val/"
    os.makedirs(output_dir, exist_ok=True)
    
    
    data_manager = Eucalyptus_openset_loader(bs=128,dataset=dataset)
    ...
    for alpha in alphas:
        for epsilon in epsilons:
            ...
            for fold in range(5):
                ...
                model.load_state_dict(torch.load(f"/home/alexandreselani/Desktop/Eucalyptus/OpenSet/Models/{dataset}/AlexNet_fold_{fold}.pt"))
                model.to(device=device)
                model.eval()

                

                train_dataloader, val_dataloader = data_manager.load_train(fold,transforms), data_manager.load_val(fold,transforms)
                ...
                for tail in tails:
                    ...
                    if matrizes_confusao_acumulada[tail] is None:
                        matriz = mc(predicts, targets_val, all_targets, [], ["Ceratocystis", "Ground", "Healthy"])
            ...
            filename_csv = f"Results_grid_eucalyptus_tail_{min_tailsize}_{max_tailsize}_eps_{epsilon}_alpha_{alpha}.csv"
...
def test_hiperparameters(alphas, epsilons, tails):
    output_dir = f"/home/alexandreselani/Desktop/pytorch-ood/pytorch-ood/experimento_eucalyptus/{dataset}/Test/"
    os.makedirs(output_dir, exist_ok=True)
        ...
        for epsilon in epsilons:
            ...
            for tail in tails:
                ...
                for fold in range(5):
                    ...
                    train_dataloader, test_dataloader = data_manager.load_train(fold, transforms), data_manager.load_test(fold, transforms)
                    ...
                    if matrizes_confusao_acumulada[tail] is None:
                        matriz = mc(predicts, targets_test, all_targets, [], ["Ceratocystis", "Ground", "Healthy"])
                        matriz.computa_matriz()
                        matrizes_confusao_acumulada[tail] = matriz
                    else:
