...
num_classes = 10
best_epoch = 76 #constatado previamente por meio de uma selecao de modelo do discriminador
bs=256
...
def create_instances(epsilon,num_classes,best_epoch):
    ...
    path_to_D = os.path.join("/home/alexandreselani/Desktop/OpenGan/OpenGAN-IC/Experimentos/Mnist_omni/OpenGan_Mnist_omniLeNet",'epoch-{}.DNet'.format(best_epoch))

    discriminator.load_state_dict(torch.load(path_to_D))

    #---------------------Carregando o classificador
    classifier = LeNet_feature_extraction(num_classes)
    classifier.load_model(torch.load(NOMES.LENET_MNIST_OMNI.value))
...
def main():
    ...
    mc.computa_matriz()
    mc.exibe_matriz(dir=os.path.join("/home/alexandreselani/Desktop/OpenGan/Resultados/Mnist_omni",model_name,f"Mnist_omniglot_eps_{epsilon}"))
