...
project_name = f'OpenGan_Eucalpytus_{dataset}' + modelFlag   # we save all the checkpoints in this directory
...
path_to_feats = os.path.join(NOMES.FEATS_DIR.value,"Eucalyptus",dataset,modelFlag) # the path to cached off-the-shelf features
...
exp_dir = os.path.join("/home/alexandreselani/Desktop/OpenGan/OpenGAN-IC/Experimentos/Eucalyptus/",dataset,modelFlag)
...
for iteration in range(5):
    ...
    data_manager = Eucalyptus_openset_loader(bs=batch_size,dataset=dataset)
    dataloader = data_manager.load_train(iteration,weights.transforms())
    

    feat_extractor = AlexNet_feature_extraction(2)
    feat_extractor.load_model(torch.load(os.path.join("/home/alexandreselani/Desktop/Eucalyptus/OpenSet/Models/",dataset,f"AlexNet_fold_{iteration}.pt")))
