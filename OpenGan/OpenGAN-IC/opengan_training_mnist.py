...
path_to_feats = '/home/alexandreselani/Desktop/OpenGan/Features_extraidas/Mnist/' # the path to cached off-the-shelf features
...
for iteration in range(5):
    ...
    save_dir_completo = os.path.join(save_dir,f"Iter_{iteration}/")

    mnist_train = torch.load(os.path.join(path_to_feats,f'Iter_{iteration}',"mnist_treino_features.pt"))
    mnist_train_features = mnist_train["features"]
    ...
    trainset_closeset = FeatDataset(data=mnist_train_features)
    ...
    for epoch in range(num_epochs):
        ...
        for i, data in enumerate(dataloader, 0):
            ...
            # Format batch
            real_cpu = data
            real_cpu = real_cpu.view(real_cpu.size(0), 512, 1, 1).to(device,dtype=torch.float32)
        ...
        if not os.path.exists(save_dir_completo):
            os.makedirs(save_dir_completo)

        path_to_save_paramOnly = os.path.join(save_dir_completo, 'epoch-{}.GNet'.format(epoch+1))
        torch.save(cur_model_wts, path_to_save_paramOnly)
        
        cur_model_wts = copy.deepcopy(netD.state_dict())
        path_to_save_paramOnly = os.path.join(save_dir_completo, 'epoch-{}.DNet'.format(epoch+1))
        torch.save(cur_model_wts, path_to_save_paramOnly)
    ...
    plt.ylabel("Loss")
    plt.legend()
    plt.savefig(os.path.join(save_dir_completo,'learningCurves_{}.png'.format(modelFlag)), bbox_inches='tight',transparent=True)
    # plt.show()
